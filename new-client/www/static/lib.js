...
function run(level) {
    window.onload = () => {
        ...
        input_backend_out = document.getElementById("input_backend_status");
...
function frame_rate_scale(x) {
    return Math.pow(x / 100, 1.5);
}
function frame_rate_scale_inv(x) {
...
function getPointerEvents(event) {
    const coalescedEvent = event;
...
class InputCaptureState {
    constructor() {
        ...
        this.keyboardButton = document.getElementById("keyboard_capture_toggle");
        this.pointerButton = document.getElementById("pointer_capture_toggle");
        this.bothButton = document.getElementById("input_capture_toggle");
    ...
    handleButton(event, keyboard, pointer) {
        event.preventDefault();
        event.stopPropagation();
        this.set(keyboard, pointer);
    }
    set(keyboard, pointer) {
    ...
    updateButton(button, active, text) {
        if (!button)
            return;
...
class Settings {
    constructor(webSocket) {
        ...
        this.frame_rate_input = document.getElementById("frame_rate");
        ...
        this.input_backend_select = document.getElementById("input_backend");
        ...
        this.checks.get("energysaving").onchange = (e) => {
            this.save_settings();
            this.toggle_energysaving(e.target.checked);
        };
    ...
    toggle_energysaving(energysaving) {
...
function init() {
    ...
    let webSocket = new WebSocket(protocol + window.location.hostname + ":" +
