...
```
git clone https://github.com/bufanyun/auroraops.git && cd auroraops
```

二、配置你的站点信息

1、服务端：
- 项目数据库文件 `storage/data/auroraops.sql` 创建数据库并导入
...
#   link: "mysql:账号:密码@tcp(127.0.0.1:3306)/数据库名?loc=Local&parseTime=true&charset=utf8mb4"
    link: "mysql:auroraops:hg123456.@tcp(127.0.0.1:3306)/auroraops?loc=Local&parseTime=true&charset=utf8mb4"
    debug: true
    Prefix: "hg_"
```
...
2、web前端：
- 配置服务端地址，包含在以下文件中：
* /auroraops/web/.env.development
* /auroraops/web/.env.production
* /auroraops/web/.env
